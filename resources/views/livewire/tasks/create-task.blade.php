...
<div>
    <x-mary-modal wire:model="createTaskModal" class="backdrop-blur">
        ...
        <x-slot name="slot" class="!text-black">
            <form class="flex flex-col gap-4" wire:submit.prevent="createTask">
                ...
                <x-mary-datetime label="Due date" placeholder="Select a date" wire:model="taskDueDate"
                    icon="o-calendar" />
                <x-mary-button label="Create" type="submit" spinner="createTask" class="btn-primary" />
            </form>
        </x-slot>

        <x-mary-button label="Cancel" wire:click="createTaskModal = false" />
    </x-mary-modal>

    <x-mary-button label="Create task" wire:click="createTaskModal = true" class="self-end btn-primary"
        icon-right="o-plus" />
</div>
